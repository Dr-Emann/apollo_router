...
The `router_http` hook runs at the raw HTTP layer before the Router pipeline; it receives the same `service` object and supports `map_request` and `map_response` like the other hooks. `router_http` runs only for **GraphQL requests** to the supergraph path; it doesn't run for the static landing page (Apollo Sandbox or homepage when enabled) or the health check endpoint, which is a separate route.

<Caution>
Using the `router_http` hook means your script runs *before* traffic shaping, limits, CSRF, and other router protections. Only use the `router_http` hook if you can accept that risk.
</Caution>
...
Router.APOLLO_COST_BY_SUBGRAPH_ESTIMATED_KEY // Context key to get the estimated cost of an operation against each subgraph
Router.APOLLO_COST_BY_SUBGRAPH_ACTUAL_KEY // Context key to get the actual cost of an operation against each subgraph
Router.APOLLO_COST_BY_SUBGRAPH_RESULT_KEY // Context key to get the cost result of an operation against each subgraph
```
