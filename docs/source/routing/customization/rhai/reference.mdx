...
The `router_http` hook runs at the raw HTTP layer before the Router pipeline; it receives the same `service` object and supports `map_request` and `map_response` like the other hooks. It runs only for **GraphQL requests** to the supergraph path. It does **not** run for the static landing page (Apollo Sandbox or homepage when enabled) or for the health check endpoint (which is a separate route).

<Caution>
Using the `router_http` hook means your script runs *before* traffic shaping, limits, CSRF, and other router protections. Only use it if you understand and accept that risk.
</Caution>
