...
Runs at the raw HTTP layer, before the main Router pipeline (and before telemetry, traffic shaping, and the Router service).

Define `router_http` if your customization must run at the earliest possible point—for example, very early request checks or logic that applies to GraphQL traffic. The RouterHttp layer runs only for **GraphQL requests**; it does **not** run for the [Apollo Sandbox](/graphos/explorer/sandbox) or homepage when enabled, or for the [health check](/graphos/routing/self-hosted/health-checks) endpoint (separate route). It uses the same `service` API as other hooks (`map_request`, `map_response`).

<Caution>
Using the `router_http` hook means your customization runs *before* traffic shaping, limits, CSRF, and other router protections. Only use it if you understand and accept that risk.
</Caution>
