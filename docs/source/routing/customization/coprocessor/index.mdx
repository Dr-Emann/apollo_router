...
As shown in the diagram above, the **RouterHttp** pipeline (when configured), `RouterService`, `SupergraphService`, `ExecutionService`, `SubgraphService`, and `ConnectorRequestService` steps of the [request-handling lifecycle](/graphos/routing/customization/rhai/#router-request-lifecycle) can send these POST requests (also called **coprocessor requests**). The RouterHttp stage runs at the raw HTTP layer before the Router pipeline, so you can inspect and modify requests at the earliest possible point (analogous to Rhai's `router_http` hook).
...
To hook into the **RouterHttp** pipeline (raw HTTP layer, before the Router pipeline) instead of or in addition to the Router stage, use the `router_http` key:

<Caution>
Using the **RouterHttp** stage means your coprocessor runs *before* traffic shaping, limits, CSRF, and other router protections. Only use it if you understand and accept that risk. Ensure your coprocessor can scale to handle load.
</Caution>
...
With this configuration, the coprocessor receives requests at the `RouterHttpRequest` and (if you add a `response` block) `RouterHttpResponse` stages.
