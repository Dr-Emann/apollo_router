...
As shown in the diagram, the **RouterHttp** pipeline (when configured), `RouterService`, `SupergraphService`, `ExecutionService`, `SubgraphService`, and `ConnectorRequestService` steps of the [request-handling lifecycle](/graphos/routing/customization/rhai/#router-request-lifecycle) can send certain POST requests, also called "coprocessor requests". The RouterHttp stage runs at the raw HTTP layer before the Router pipeline, so you can inspect and modify requests at the earliest possible moment (analogous to Rhai's `router_http` hook).
...
If your coprocessor hooks into your router's `ConnectorRequestService`, the router sends a separate coprocessor request _for each connector HTTP request_. If your router makes multiple REST API calls via [connectors](/graphos/schema-design/connectors/) to resolve a client operation, it sends a separate coprocessor request for each call. Each coprocessor request includes the connector source [name](#servicename) and the target [URL](#uri).
...
To hook into the **RouterHttp** pipelin—which exists in the raw HTTP layer, before the Router pipeline—instead of or in addition to the Router stage, use the `router_http` key:

<Caution>
Using the **RouterHttp** stage means your coprocessor runs *before* traffic shaping, limits, CSRF, and other router protections. Only use the **RouterHttp** stage if you accept that risk and your coprocessor can scale to handle the load.
</Caution>
...
With that configuration, the coprocessor receives requests at the `RouterHttpRequest` and, if you add a `response` block, `RouterHttpResponse` stages.
