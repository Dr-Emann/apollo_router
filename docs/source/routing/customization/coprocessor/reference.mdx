...
- `RouterHttpRequest`: The **RouterHttp** pipeline has just received a raw HTTP request (before the Router pipeline). This runs at the earliest possible point, analogous to Rhai's `router_http` hook.
- `RouterHttpResponse`: The **RouterHttp** pipeline is about to send the HTTP response to the client (before the Router pipeline).
...
- `ConnectorRequest`: The `ConnectorRequestService` is about to send an HTTP request to a REST API.
- `ConnectorResponse`: The `ConnectorRequestService` has just received an HTTP response from a REST API.

<Caution>
Using the **RouterHttp** stage (`RouterHttpRequest` / `RouterHttpResponse`) means your coprocessor runs *before* traffic shaping, limits, CSRF, and other router protections. Only use it if you understand and accept that risk. Ensure your coprocessor can scale to handle load.
</Caution>
...
- For connector stages, `body` is a JSON _object_ for both requests and responses when the body is valid JSON, or a JSON _string_ if the request body is not valid JSON.
  - If the request body is valid JSON, it is sent as a parsed JSON object. Otherwise, it is sent as a raw string.
  - The response body is the parsed JSON data or an error object.
...
The name of the subgraph or connector source that this coprocessor request pertains to.

This value is present for coprocessor requests from the router's `SubgraphService` and connector stages.
...
When `stage` is `SubgraphRequest` or `ConnectorRequest`, this is the full URI of the subgraph or API endpoint the router will query.
...
- `apollo::demand_control::estimated_cost_by_subgraph` - Map of estimated cost of the requests to be sent to the subgraphs, by subgraph
- `apollo::demand_control::actual_cost_by_subgraph` - Map of actual calculated cost of the responses returned by the subgraphs, by subgraph
- `apollo::demand_control::result_by_subgraph` - Map of result by subgraph, `COST_OK` if allowed, and `COST_TOO_EXPENSIVE` if rejected due to cost limits
...
```json
{
  ...
  "context": {
    "entries": {
      "apollo::supergraph::operation_name": "GetUsers",
      "apollo::telemetry::client_name": "web-app"
    }
  },
  "uri": "https://api.example.com/v1/users",
  "method": "GET",
  "serviceName": "my_subgraph_name.my_source_name"
}
```
