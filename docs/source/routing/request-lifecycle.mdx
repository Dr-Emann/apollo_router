...
* The **RouterHttp** pipeline runs first at the raw HTTP layer. Rhai scripts, native plugins, the external coprocessor (when configured), and built-in features (such as license enforcement) can hook in here before the request reaches the Router service. Only **GraphQL requests** pass through this layer; the [Apollo Sandbox](/graphos/explorer/sandbox) (or homepage) when enabled and the [health check](/graphos/routing/self-hosted/health-checks) endpoint are served separately and do not go through RouterHttp. HTTP server layers (e.g. decompression, tracing) may run before the RouterHttp pipeline.
...
* **RouterHttp plugins** run first (Rhai, native plugins, coprocessor, license enforcement).
* Then the **Router pipeline** runs: first the built-in plugins that have a `router_service` hook (for example, telemetry, traffic shaping, limits, CSRF, fleet detector, health check, headers), then user plugins, then content negotiation and the Router core.
...
2. GraphQL requests pass through the **RouterHttp** pipeline at the raw HTTP layer, where plugins (Rhai, native, license enforcement) can run. The static landing page (Sandbox/homepage) and the health check endpoint do not go through this layer; they are served separately. The HTTP server then transforms the HTTP request into a `RouterRequest` containing HTTP headers and the request body as a stream of byte arrays.
...
* **RouterHttp** (`router_http_service`) is the top-level hook and runs before all Router-pipeline plugins (telemetry, traffic shaping, limits, CSRF, and other built-in plugins with a `router_service` hook). Use **RouterHttp** when you need the earliest possible point—for example, before rate limiting or telemetry.
...
<Caution>
Using the **RouterHttp** hook means your code runs *before* traffic shaping, limits, CSRF, and other router protections. Only use it if you understand and accept that risk. If you use a coprocessor at RouterHttp, ensure you can scale it to handle load.
</Caution>
