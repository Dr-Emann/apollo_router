...
* **RouterHttp** (`router_http_service`) is the top-level hook and runs before all Router-pipeline plugins, like telemetry, traffic shaping, limits, CSRF, and other built-in plugins with a `router_service` hook. Use **RouterHttp** when you need entry at the earliest possible moment (for example, before rate limiting or telemetry).
